        ...
        section.innerHTML = `
            ...
            <div class="channel-table" style="${isEnabled ? '' : 'opacity:0.5; pointer-events:none; transition: opacity 0.3s;'}">
                ...
                <div class="channel-row">
                    ...
                    <div class="channel-cell">${buildSizeSelect(provider, 'text', textDefaults.size, textDefaults.model)}</div>
                    <div class="channel-cell">${buildQualitySelect(provider, textDefaults.quality, 'text')}</div>
                    <div class="channel-cell">${buildCountSelect(provider, textDefaults.n, 'text')}</div>
                </div>
                <div class="channel-row">
                    ...
                    <div class="channel-cell">${buildSizeSelect(provider, 'edit', editDefaults.size, editDefaults.model)}</div>
                    <div class="channel-cell">${buildQualitySelect(provider, editDefaults.quality, 'edit')}</div>
                    <div class="channel-cell">${buildCountSelect(provider, editDefaults.n, 'edit')}</div>
                </div>
                <div class="channel-row">
                    ...
                    <div class="channel-cell">${buildModelSelect(provider, 'blend', blendDefaults.model)}</div>
                    <div class="channel-cell">${buildSizeSelect(provider, 'blend', blendDefaults.size, blendDefaults.model)}</div>
                    <div class="channel-cell">${buildQualitySelect(provider, blendDefaults.quality, 'blend')}</div>
                    <div class="channel-cell">${buildCountSelect(provider, blendDefaults.n, 'blend')}</div>
                </div>
            </div>
        `;
...
function buildSizeSelect(provider, task, currentValue, currentModel) {
    ...
    for (const s of sizes) {
        ...
        const label = isDoubao ? formatDoubaoSizeLabel(s) : s;
    ...
    if (!hasSelection && sizes.length > 0) {
        ...
        html = `<select class="form-control" data-provider="${provider.name}" data-task="${task}" data-field="size">`;
...
function updateDoubaoSizeOptions(modelSelect) {
    ...
    // 找到对应的尺寸下拉框
    const sizeSelect = document.querySelector(`select[data-provider="${providerName}"][data-task="${task}"][data-field="size"]`);
